...
import { createJobListing, updateJobListing, publishJobListing } from "../actions/actions"
...
export function JobListingForm({
  ...
  async function onSubmitAndPublish(data: z.infer<typeof jobListingSchema>) {
    if (jobListing) {
      // For existing job listings, just update and publish
      const res = await updateJobListing(jobListing.id, data)
      if (res.error) {
        toast.error(res.message)
      } else {
        const publishRes = await publishJobListing(jobListing.id)
        if (publishRes.error) {
          const errorMessage = 'message' in publishRes ? publishRes.message : "Unknown error"
          toast.error("Job listing updated but failed to publish: " + errorMessage)
        } else {
          toast.success("Job listing updated and published successfully!")
        }
      }
    } else {
      // For new job listings, create first then publish
      const res = await createJobListing(data)
      if (res.error) {
        toast.error(res.message)
      }
      // Note: createJobListing redirects on success, so we won't reach the publish logic
      // The publish should be handled after the redirect
    }
  }
        ...
        <div className="flex gap-2">
          ...
          >
            <LoadingSwap isLoading={form.formState.isSubmitting}>
              {jobListing ? "Update Draft" : "Save as Draft"}
            </LoadingSwap>
          </Button>
          {!jobListing && (
            <Button
