...
import { hasOrgUserPermission as hasCustomPermission } from "@/services/clerk/lib/customPermissions"
...
export async function createJobListing(
  ...
  if (
    orgId == null ||
    userId == null ||
    !(await hasCustomPermission("org:job_listings:create"))
  ) {
    return {
      error: true,
  ...
  // Auto-sync organization and user to ensure they exist in database
  try {
    // Get organization data from sessionClaims
    const { sessionClaims } = await auth()
    const orgData = sessionClaims?.o as any
    const orgName = orgData?.slg ? orgData.slg.replace(/-[0-9]+$/, '') : `Organization-${orgId.slice(-6)}`
    ...
    // Get user data from sessionClaims
...
export async function updateJobListing(
  ...
  if (
    orgId == null ||
    !(await hasCustomPermission("org:job_listings:update"))
  ) {
    return {
      error: true,
...
export async function publishJobListing(id: string) {
  const error = {
    error: true,
    message: "You don't have permission to publish this job listing",
  }
  const { orgId } = await getCurrentOrganization()
  ...
  if (
    !(await hasCustomPermission("org:job_listings:change_status")) ||
  ...
  await updateJobListingDb(id, {
    status: "published",
    postedAt: jobListing.postedAt == null ? new Date() : undefined,
  })
...
export async function deleteJobListing(id: string) {
  ...
  if (!(await hasCustomPermission("org:job_listings:delete"))) {
    return error
  }
